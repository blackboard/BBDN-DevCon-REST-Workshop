...
public class RestDemo {
	...
	public static void main(String[] args) {
		...
		// Course object
		if( OPER_ALL || OPER_COURSE) {
			cr = courseHandler.createObject(token.getToken(), ds.getId(), tm.getId());
			log.debug("Create CS: " + cr.toString());
			cr = courseHandler.readObject(token.getToken());
			log.debug("Read CS: " + cr.toString());
			cr = courseHandler.updateObject(token.getToken(), ds.getId(), tm.getId());
			log.debug("Update CS: " + cr.toString());
		}
		
		// User object
		if( OPER_ALL || OPER_USER) {
			us = userHandler.createObject(token.getToken(), ds.getId());
			log.debug("Create US: " + us.toString());
			us = userHandler.readObject(token.getToken());
			log.debug("Read US: " + us.toString());
			us = userHandler.updateObject(token.getToken(), ds.getId());
			log.debug("Update US: " + us.toString());
		}
			
		//Membership Object
		if( OPER_ALL || OPER_MEMBERSHIP) {	
			mb = membershipHandler.createObject(token.getToken(), ds.getId(), us.getId(), cr.getId());
			log.debug("Create MS: " + mb.toString());
			mb = membershipHandler.readObject(token.getToken());
			log.debug("Read MS: " + mb.toString());
			mb = membershipHandler.updateObject(token.getToken(), ds.getId(), us.getId(), cr.getId());
			log.debug("Update MS: " + mb.toString());
		}
		
		// Delete objects	
		if( OPER_ALL || OPER_MEMBERSHIP) {
			result = membershipHandler.deleteObject(token.getToken());
			log.debug("Delete MS: " + result);
		}
		
		if( OPER_ALL || OPER_USER) {
			result = userHandler.deleteObject(token.getToken());
			log.debug("Delete US: " + result);
		}
		
		if( OPER_ALL || OPER_COURSE) {
			result = courseHandler.deleteObject(token.getToken());
			log.debug("Delete CS: " + result);
		}
